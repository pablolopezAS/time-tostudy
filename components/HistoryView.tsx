...
const HistoryView: React.FC<HistoryProps> = ({ sessions, subjects }) => {
  ...
  const monthlyPieData = useMemo(() => {
    ...
    weeksData.forEach(w => {
      Object.entries(w.subjects).forEach(([subId, time]) => {
        subjectTotals[subId] = (subjectTotals[subId] || 0) + (time as number);
      });
    });
    return Object.entries(subjectTotals).map(([subId, time]) => ({
      label: getSubject(subId)?.name || 'Desconocida',
      value: (time as number) / 3600,
      color: getSubject(subId)?.color || '#94a3b8'
    }));
  }, [weeksData, subjects]);

  const weeklyPieData = useMemo(() => {
    if (!activeWeek) return [];
    return Object.entries(activeWeek.subjects).map(([subId, time]) => ({
      label: getSubject(subId)?.name || 'Desconocida',
      value: (time as number) / 3600,
      color: getSubject(subId)?.color || '#94a3b8'
    }));
  ...
  const evolutionData = useMemo(() => {
    ...
    return subjects.map(sub => ({
      label: sub.name,
      values: weeksData.map((w, i) => ({
        label: `S${i + 1}`,
        value: (w.subjects[sub.id] || 0) / 3600
      }))
    }));
  }, [weeksData, subjects]);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-10 pb-40">
          ...
          {calendarDays.map((date, i) => {
            ...
            return (
              <button
                ...
                className={`relative aspect-square sm:aspect-[4/5] rounded-xl sm:rounded-2xl border transition-all flex flex-col p-2 sm:p-3 print:bg-white print:border-slate-100 ${isSelected ? 'bg-indigo-500 text-white border-indigo-500 shadow-lg print:border-indigo-500' :
      ...
      {/* ANALÍTICA MENSUAL (PIE CHART) */}
      <section className="space-y-4 print:page-break">
        <div className="flex items-center gap-2 px-1">
          <BarChart3 size={18} className="text-indigo-400" />
          <h3 className="text-sm font-black text-slate-700 uppercase tracking-widest">Distribución del Mes</h3>
        </div>
        <div className="glass p-8 rounded-[2.5rem] border border-white/60 flex flex-col items-center justify-center bg-white/40 print:shadow-none print:border-slate-200">
          {monthlyPieData.length > 0 ? (
            <PieChart data={monthlyPieData} size={220} />
          ) : (
            <div className="h-[220px] flex flex-col items-center justify-center text-center text-slate-300">
              <BarChart3 size={24} className="mb-2" />
              <p className="text-[10px] font-bold uppercase tracking-widest">Sin datos este mes</p>
            </div>
          )}
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 gap-3 print:grid-cols-1">
          {weeksData.map((week) => {
            ...
            return (
              <div
                key={week.key}
                className={`glass rounded-3xl border overflow-hidden transition-all print:border-slate-200 print:bg-white ${isSelected ? 'border-indigo-500 ring-2 ring-indigo-500/20' : 'border-white/60'
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {selectedWeekKey && weeklyPieData.length > 0 && (
            <div className="glass p-8 rounded-[2.5rem] border border-white/60 flex flex-col items-center justify-center bg-white/40 mb-6 print:shadow-none print:border-slate-200">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 text-center">Distribución de la Semana</p>
              <PieChart data={weeklyPieData} size={180} />
            </div>
          )}
      ...
      {/* EVOLUCIÓN SEMANAL (MULTILINE CHART) */}
      <section className="space-y-4 pt-10 pb-10 print:page-break">
        <div className="flex items-center gap-2 px-1">
          <TrendingUp size={18} className="text-emerald-400" />
          <h3 className="text-sm font-black text-slate-700 uppercase tracking-widest">Evolución por Asignatura</h3>
        </div>
        <div className="glass p-8 rounded-[3rem] border border-white/60 bg-white/40 print:border-slate-200 print:bg-white overflow-x-auto">
